...
//     (\`new BreakingBadQuotesSDK(...)\` / \`BreakingBadQuotesSDK.test(...)\` become
//     \`BreakingBadQuotesSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'BreakingBadQuotesSDK'
...
// rewrite real client construction (\`new BreakingBadQuotesSDK(...)\` and any
// \`BreakingBadQuotesSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+BreakingBadQuotesSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/BreakingBadQuotesSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
