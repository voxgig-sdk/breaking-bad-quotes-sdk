// Command BreakingBadQuotes-cli is a CLI + REPL for the BreakingBadQuotes SDK,
...
const prompt = "breaking-bad-quotes"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      BREAKING_BAD_QUOTES_APIKEY (api key), BREAKING_BAD_QUOTES_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: BREAKING_BAD_QUOTES_APIKEY carries the API key and
	// BREAKING_BAD_QUOTES_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("BREAKING_BAD_QUOTES_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("BREAKING_BAD_QUOTES_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewBreakingBadQuotesSDK(opts)
