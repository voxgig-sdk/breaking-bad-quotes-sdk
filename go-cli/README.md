...
AQL-driven command-line client **and** interactive REPL for the BreakingBadQuotes
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/breaking-bad-quotes-cli)
make build

# 2. See usage (words, entities, env vars)
./breaking-bad-quotes-cli --help
...
./breaking-bad-quotes-cli list quote
./breaking-bad-quotes-cli load 1 quote            # {id:1} shorthand
./breaking-bad-quotes-cli load '{id:1}' quote       # explicit match map

# 5. Override the API base URL for a single call
BREAKING_BAD_QUOTES_BASE=https://api.example.com ./breaking-bad-quotes-cli list quote

# 6. No arguments -> interactive REPL
./breaking-bad-quotes-cli
breaking-bad-quotes> list quote
breaking-bad-quotes> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/breaking-bad-quotes-cli
   ```
...
3. **Run a query.** Evaluate an AQL expression against the API (or run with no
   ...
   ```sh
   ./dist/*/breaking-bad-quotes-cli list quote
   ```
...
```sh
./breaking-bad-quotes-cli load 1 quote          # scalar shorthand for {id:1}
./breaking-bad-quotes-cli load '{id:1}' quote     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `breaking-bad-quotes>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/breaking-bad-quotes-cli`. |
...
(`./breaking-bad-quotes-cli <expr>`) and interactively (the REPL), and expressions compose the
