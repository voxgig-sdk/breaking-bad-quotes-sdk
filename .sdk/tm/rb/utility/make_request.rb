...
module BreakingBadQuotesUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = BreakingBadQuotesResponse.new({})
    ...
    if fetch_err
      ...
      response = BreakingBadQuotesResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = BreakingBadQuotesResponse.new(fetched)
    else
