...
// BreakingBadQuotes SDK feature test
//
...
class FtCtrl extends BreakingBadQuotesControl
{
...
class FtClient
{
    ...
    private BreakingBadQuotesUtility $utility;

    public function __construct(BreakingBadQuotesUtility $utility, array $options)
    {
    ...
    public function get_utility(): BreakingBadQuotesUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (BreakingBadQuotesContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility = new BreakingBadQuotesUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (BreakingBadQuotesContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = BreakingBadQuotesConfig::make_feature($name);
    ...
    private static function buildUrl(BreakingBadQuotesSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof BreakingBadQuotesError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new BreakingBadQuotesResponse($fetched);
        }
    ...
    private function fail(BreakingBadQuotesContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (BreakingBadQuotesContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
