# breaking-bad-quotes-mcp

[MCP](https://modelcontextprotocol.io) server exposing the BreakingBadQuotes SDK as
two agent tools — `breaking-bad-quotes_list` and `breaking-bad-quotes_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/breaking-bad-quotes-mcp)
make build

# 2. Provide credentials via the environment
export BREAKING_BAD_QUOTES_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user breaking-bad-quotes \
  -- /absolute/path/to/breaking-bad-quotes-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./breaking-bad-quotes-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// breaking-bad-quotes_list: first page of records
{ "entity": "quote" }
{ "entity": "quote", "query": { } }

// breaking-bad-quotes_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/breaking-bad-quotes-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user breaking-bad-quotes \
     -- "$PWD"/dist/*/breaking-bad-quotes-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `breaking-bad-quotes_list` and `breaking-bad-quotes_load` tools now appear
   in new sessions. Ask the agent to *"list quote using breaking-bad-quotes"*
   and it calls `breaking-bad-quotes_list` with `{"entity":"quote"}`.
...
```sh
export BREAKING_BAD_QUOTES_APIKEY=sk_live_xxx            # API key
export BREAKING_BAD_QUOTES_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `breaking-bad-quotes_list` tool
...
### Call the `breaking-bad-quotes_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `breaking-bad-quotes_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `breaking-bad-quotes_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `BREAKING_BAD_QUOTES_APIKEY` | API key sent with every request. |
| `BREAKING_BAD_QUOTES_BASE` | Optional override of the API base URL. |
...
```sh
./breaking-bad-quotes-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"breaking-bad-quotes_load","arguments":{"entity":"quote","query":{"id":1}}}}'
```
