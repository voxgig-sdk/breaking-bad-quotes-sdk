# BreakingBadQuotes SDK Quote entity
...
class QuoteEntity
  ...
  def data_set(args)
    if args
      @_data = BreakingBadQuotesHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = BreakingBadQuotesHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Quote, Hash] the loaded Quote; raises BreakingBadQuotesError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = BreakingBadQuotesHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Quote>, Array] the matching Quote items; raises BreakingBadQuotesError on failure
