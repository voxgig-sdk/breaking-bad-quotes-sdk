...
// BreakingBadQuotes SDK retry feature
...
class BreakingBadQuotesRetryFeature extends BreakingBadQuotesBaseFeature
{
    ...
    public function init(BreakingBadQuotesContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (BreakingBadQuotesContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(BreakingBadQuotesContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(BreakingBadQuotesContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
