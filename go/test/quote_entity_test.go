...
func TestQuoteEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set BREAKINGBADQUOTES_TEST_QUOTE_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func quoteBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("BREAKINGBADQUOTES_TEST_QUOTE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["BREAKINGBADQUOTES_TEST_QUOTE_ENTID"])
	...
	if env["BREAKINGBADQUOTES_TEST_LIVE"] == "TRUE" {
		...
		client = sdk.NewBreakingBadQuotesSDK(core.ToMapAny(mergedOpts))
	}
